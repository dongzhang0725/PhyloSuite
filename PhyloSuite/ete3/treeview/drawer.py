...
from .qt4_gui import _GUI, _BasicNodeActions
...
def show_tree(t, layout=None, tree_style=None, win_name=None, parent=None):
    """ Interactively shows a tree."""
    scene, img = init_scene(t, layout, tree_style)
    # control show leaf name, occupied colomn 0, # 删掉node的球形
    # img.show_leaf_name = False
    # img.layout_fn = lambda node: \
    #     [faces.add_face_to_node(faces.AttrFace("name"), node, column=0),
    #      node.img_style.__setitem__("size", 0)] if node.is_leaf() \
    #         else node.img_style.__setitem__("size", 0)
    # img.layout_fn = lambda node: node.img_style.__setitem__("size", 0) # cause layout_fn not work latter
    # img.show_leaf_name = False
    ...
    treesuite_flag = True if win_name == "treesuite-ETE" else False
    mainapp = _GUI(scene, mcmctree_flag, treesuite_flag)
    if win_name:
        mainapp.setObjectName(win_name)
    mainapp.setParent(parent)
    mainapp.setWindowFlags(Qt.Window | Qt.WindowMinMaxButtonsHint | mainapp.windowFlags())
    mainapp.show()
