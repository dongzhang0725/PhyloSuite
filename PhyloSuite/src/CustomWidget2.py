...
class MyMappingTableModel(MyTableModel):
    def __init__(self, datain, headerdata, column_fixed=[], parent=None):
        """
        Args:
            datain: a list of lists\n
            headerdata: a list of strings
    ...
    def flags(self, index):
        # value = self.arraydata[index.row()][index.column()]
        if index.column() in self.column_fixed:
            return Qt.ItemIsEnabled
        # elif type(value) == bool:
        #     return Qt.ItemIsEnabled | Qt.ItemIsUserCheckable
        # elif (type(value)==str and re.search("^#[0-9ABCDEFabcdef]{6}$", str(value))) \
...
class MyListWidget(QListWidget):
    ...
    def dropEvent(self, event):
        if hasattr(self.parent, "update_face_pos") and callable(self.parent.update_face_pos):
            self.parent.update_face_pos()

    def removeItem(self, mode="ete"):
        sender = self.sender()
        ...
        if reply == QMessageBox.Ok:
            ...
            if mode == "ete":
                ...
                list_elements = self.parent.list_faces.pop(row)
